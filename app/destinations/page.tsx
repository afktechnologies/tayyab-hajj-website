...
async function getDestinations(): Promise<{ destinations: IDestination[] | null; error?: string }> {
  try {
    const response = await fetch(`${process.env.BASE_URL}/api/admin/destinations`, {
      cache: "no-store",
    });
    if (!response.ok) {
      return { destinations: null, error: "Failed to fetch destinations" };
    }
    const data = await response.json();
    return { destinations: data.destinations || [] };
  } catch (error) {
    return { destinations: null, error: "Server error" };
  }
}
...
export default async function DestinationsPage() {
  ...
  const meccaSites = destinations.filter((site) => site.city === "Mecca");
  const madinahSites = destinations.filter((site) => site.city === "Madinah");
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <Tabs defaultValue="mecca" className="w-full">
            <TabsList className="grid w-full grid-cols-2 max-w-md mx-auto mb-12 rounded-lg p-1 bg-transparent">
              <TabsTrigger
                value="mecca"
